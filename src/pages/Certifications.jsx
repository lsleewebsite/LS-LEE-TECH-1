...
export default function Certifications() {
  ...
  useEffect(() => {
    async function fetchCerts() {
      try {
        ...
        if (!res.ok) {
          throw new Error(`Certifications API error: ${res.status}`)
        }
  ...
  useEffect(() => {
    if (certs.length > 0 && activeIndex >= certs.length) {
      setActiveIndex(0)
    }
  }, [certs, activeIndex])
  ...
  const prev = () => {
    ...
    setActiveIndex((i) => (i === 0 ? certs.length - 1 : i - 1))
  }

  const next = () => {
    if (certs.length === 0) return
    setActiveIndex((i) => (i === certs.length - 1 ? 0 : i + 1))
  }
  ...
  return (
    <div>
      <section
        style={{
          padding: isMobile ? '60px 20px 40px' : '100px 32px 80px',
          background: '#0F172A',
          color: '#FFF',
      ...
      >
        <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
          ...
          >
            ...
            >
              Industry-recognised certifications demonstrating our commitment to
              quality, safety, and technical excellence.
            </p>
          </motion.div>
        </div>
      </section>
      ...
      <section
        style={{
          padding: isMobile ? '48px 20px' : '100px 32px',
          background: '#F1F5F9',
        }}
      >
              ...
              >
                ...
                <div
                  style={{
                    position: 'relative',
                    width: '100%',
                    maxWidth: isMobile ? '280px' : '1100px',
                  ...
                  {certs.map((cert, i) => {
                    ...
                    return (
                      <motion.div
                        key={cert.id}
                        onClick={() =>
                          isActive ? setSelectedCert(cert) : setActiveIndex(i)
                        }
                        animate={{
